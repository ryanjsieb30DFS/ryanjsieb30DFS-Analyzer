...
def run_analysis(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"8. `## Optimizer build rules` — the LAST section: 4–8 CONCISE rules (no filler) that turn "
        f"this strategy into constraints the user copies STRAIGHT into the Grader's Lineup Builder. "
        f"Write each in the Builder's EXACT vocabulary so it maps 1:1 to a dropdown — use ONLY these "
        f"rule types and terms:\n"
        f"   - **Salary band:** `Min $X / Max $Y`.\n"
        f"   - **Player Groups:** `At Least|At Most|Exactly N from: <player names>` — the slate's "
        f"core / leverage sets from Top plays + Leverage & fades.\n"
        f"   - **Restrictions:** `At Least|At Most|Exactly N with [Proj|Ceiling|Own %|Salary|"
        f"Starting Pos] [Less Than|Greater Than|Equals|Between] <value>` — encode the framework + "
        f"sharp-playbook structure (e.g. a leverage piece → `At Least 1 with Own % Less Than 15`; a "
        f"punt cap → `At Most 1 with Salary Less Than 7000`; a ceiling floor → `At Least N with "
        f"Ceiling Greater Than <value>`).\n"
        f"   - **IF-THEN** (only if a real conditional exists): `IF Contains ANY {{players}} THEN "
        f"At Least|At Most|Exactly N of {{players}}`.\n"
        f"   - **Min Uniques / per-player Exposure %** — only if the slate calls for it.\n"
        f"   Group by rule type, one rule per line, each with a ≤10-word why. Every threshold must "
        f"be a real number from the projections. Use ONLY the stats/comparators/quantifiers listed — "
        f"nothing else exists in the Builder. Name players/constraints only; NEVER assemble a lineup.\n\n"
