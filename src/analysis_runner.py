...
def run_analysis(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"tendencies` section, surface it here as a tension too — 'the field piles into X/Y in your "
        f"<type> contests (in 3 of 4) — that's where leverage-away lives,' citing the counts. Use NO "
        ...
        f"differentiation, all-unique). No restatement of the plays above.\n\n"
...
def run_player_pool(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"- **Field-crowds flag:** if the bundle at `{bundle_path}` has a `## Field tendencies` "
        f"section, append a small `(field crowds)` note in the write-up of any listed player that "
        f"appears in its reliably-crowded list — a heads-up that the field piles in, not a fade "
        f"command.\n"
