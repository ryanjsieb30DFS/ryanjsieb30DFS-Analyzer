...
def run_analysis(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"8. `## Optimizer build rules` — the LAST section: 3–6 CONCISE rules the user copies "
        f"STRAIGHT into the Grader's Lineup Builder. These are a FAITHFUL TRANSLATION of the "
        f"strategy YOU JUST WROTE + the uploaded data — NOT generic heuristics.\n"
        f"   HARD RULES for this section:\n"
        f"   (a) **Every rule must trace to a specific line in your `## Decisions` / `## Leverage & "
        f"fades` / `## Top plays` above, or to a cited data point (an article table / projection "
        f"number).** Do NOT introduce a constraint the strategy body doesn't support. If you can't "
        f"tie a rule to the text above, don't write it.\n"
        f"   (b) **SALARY comes from THIS slate's data, never a generic 'max salary'.** Read the "
        f"optimal-lineup salary evidence in the articles (or the winning structure) and set the band "
        f"to it. If the strategy body says salary is non-binding / value cars matter, use a LOW or no "
        f"floor — NEVER a floor that would exclude the slate's actual optimal lineups. (Maxing salary "
        f"is a golf/MMA trait; it is WRONG for NASCAR PD builds and any slate whose optimals don't "
        f"max out.) The salary rule's number and its stated reason MUST agree.\n"
        f"   (c) **Consistency:** before writing each rule, confirm it does not contradict the "
        f"strategy body or another rule (no 'non-binding' + a high floor; don't over-constrain a "
        f"roster into infeasibility).\n"
        f"   (d) **Represent the body's actual player reads:** Player Groups use the exact players "
        f"the Decisions/Leverage name; a headline article play you deprioritized (e.g. a named "
        f"'GPP Fav' that's chalk) is represented honestly — in a group or explicitly noted as "
        f"chalk/optional — never silently dropped.\n"
        f"   Write each in the Builder's EXACT vocabulary (maps 1:1 to a dropdown); ONLY these rule "
        f"types exist:\n"
        f"   - **Salary band:** `Min $X / Max $Y` (from the data per (b)).\n"
        f"   - **Player Groups:** `At Least|At Most|Exactly N from: <player names>`.\n"
        f"   - **Restrictions:** `At Least|At Most|Exactly N with [Proj|Ceiling|Own %|Salary|"
        f"Starting Pos] [Less Than|Greater Than|Equals|Between] <value>` (value = a real projection "
        f"number; e.g. a leverage piece → `At Least 1 with Own % Less Than 10`).\n"
        f"   - **IF-THEN** (only for a real conditional): `IF Contains ANY {{players}} THEN "
        f"At Least|At Most|Exactly N of {{players}}`.\n"
        f"   - **Min Uniques / per-player Exposure %** — only if the slate calls for it.\n"
        f"   Group by type, one rule per line + a ≤10-word why that names the Decision/data it comes "
        f"from. Use ONLY the stats/comparators/quantifiers listed — nothing else exists in the "
        f"Builder. Name constraints only; NEVER assemble a lineup.\n\n"
