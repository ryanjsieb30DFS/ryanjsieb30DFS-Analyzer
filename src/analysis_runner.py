...
def run_analysis(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"- **Plain language, complete sentences:** the user must understand every line with ZERO "
        f"decoding. 1–3 full, simple sentences per bullet — longer and clearer ALWAYS beats shorter "
        f"and cryptic. No compressed shorthand strings, no telegraphic fragments. Bold the "
        f"player/decision.\n"
        f"- **ONE IDEA PER SENTENCE. Aim for ~20 words, hard stop at 30.** This is the single most "
        f"important rule and the one most often broken. Do NOT chain clauses with semicolons, dashes "
        f"or 'and' into 60-word sentences. Do NOT stack more than TWO numbers in one sentence — split "
        f"it. Write like you are explaining the slate out loud to a smart 10-year-old who has never "
        f"played DFS: short sentences, ordinary words, one fact at a time. A reader should never have "
        f"to re-read a sentence to parse it. Prefer four short sentences over one dense one, even "
        f"though that is longer — length is fine, density is not.\n"
        ...
        f"- **Translate EVERY DFS term the first time it appears in each section**, not just vendor "
        f"jargon. That includes: place differential, dominator, ownership, chalk, leverage, "
        f"anchor-equivalence, dupe/duplication, fish trap, own-per-slot, ceiling, floor. Example: "
        f"'place differential — the points a driver earns just for finishing ahead of where he "
        f"started'. Say it in the same sentence, every section, every time.\n"
        f"- **Translate vendor jargon INLINE, every time:** never print a vendor term (coffin, dock, "
        f"boost, sim-optimal, steam, exposure) without saying what it means in the SAME sentence — "
        f"e.g. 'ETR is playing him 8 points MORE than the field expects (coffin +8.2)' or 'the model "
        f"trims his projection for course fit more than anyone's (the #1 course-fit dock)'. The "
        f"reader should never need a glossary or have to ask what a line means.\n"
        ...
        f"6. `## Leverage & fades` — two tight parts:\n"
        ...
        f"7. `## Key themes` — ≤3 bullets: the structural storylines + where the sources DISAGREE "
...
def run_grade(slug: str, contest_label: str, sport: str, lineups_text: str) -> dict:
    ...
    prompt = (
        ...
        f"Plain, complete sentences — clear beats short. NO play/fade commands, NO alternative "
...
def run_player_pool(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"+ the key risk or condition. Longer and clearer ALWAYS beats shorter and cryptic — the "
        f"user must understand every write-up with zero decoding. "
        f"**ONE IDEA PER SENTENCE, ~20 words, hard stop at 30.** Do not chain clauses with "
        f"semicolons or dashes into one long sentence, and never stack more than two numbers in a "
        f"sentence — split it. Write as if explaining the player out loud to a smart 10-year-old. "
        f"NEVER print a vendor term "
        f"(coffin, dock, boost, sim-optimal, steam) without translating it in the same sentence — "
        f"e.g. 'ETR is playing him 8 points MORE than the field expects (coffin +8.2)'. Translate "
        f"every DFS term on first use too (place differential, dominator, chalk, leverage, "
        f"ceiling, floor). NEVER print an internal source code such as 'image-35', 'DDD' or 'DFR' — "
