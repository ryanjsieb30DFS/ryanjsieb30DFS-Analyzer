...
def run_analysis(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"(own/slot, leverage rate, anchor discipline, uniqueness — the OBSERVED numbers; when the "
        f"'Observed on your logged ... slates' line exists with n>=2, ITS own/slot is the number "
        f"you quote, never the blended envelope's) applied "
        ...
        f"must clear.\n"
        f"   Close with the sharp-envelope target in ONE line — the own/slot from the 'Observed on "
        f"your logged ... slates' line when it exists with n>=2 (the blended envelope is an archive "
        f"seed and under-states it), citing the most relevant named pro from `## Shark reality` as "
        f"coaching where one exists.\n"
...
def build_player_pool_prompt(full, removed: list, strategy_note: str, slug: str,
    ...
    prompt = (
        ...
        + (
            f"THIS IS THE NFL CLASSIC POOL, BUILT POSITION BY POSITION (user process 9/12/26: "
            f"the pool is finalized Saturday night / Sunday morning from the preliminary "
            f"projections, QUARTERBACKS FIRST, then RB, WR, TE, DST; the builds are finalized "
            f"Sunday ~11:30 ET after the injury reports). **This board IS the pool**: every "
            f"player you tier Core / Good / Okay is IN the pool the sim builds from and every "
            f"Fade is OUT — nobody outside the pool is ever built. So the Fade tier is a real "
            f"decision, not a soft label: tier Fade only what should NOT be built this week, "
            f"and keep enough depth at every position for a 9-slot lineup with stacks (at "
            f"least 4 QBs, 8 RBs, 12 WRs, 5 TEs, 4 DSTs in the pool unless the slate is tiny).\n"
            f"Write `{out_path}` as:\n"
            f"- A one-line header `# {contest_label} — Player pool` and one sentence saying "
            f"this is the drafted pool, walked QB → RB → WR → TE → DST, for the user to "
            f"approve or flip.\n"
            f"- Then FIVE sections in this exact order, headed `## QB`, `## RB`, `## WR`, "
            f"`## TE`, `## DST`. Each section: (1) 1–3 plain sentences on how this position "
            f"plays this week — for QB, name each game worth building around and the story it "
            f"pays in (shootout, blowout, slog), because the QBs you keep define which games "
            f"the RB / WR / TE / DST sections stack into; for the other positions say which of "
            f"those games each pooled player rides. (2) A **ranked Markdown table** of EVERY "
            f"player at that position, best to worst, rank 1..N WITHIN the position. **Lead "
            f"with the DATA; the tier is the LAST column.** Columns EXACTLY: "
            if is_nfl_classic else
