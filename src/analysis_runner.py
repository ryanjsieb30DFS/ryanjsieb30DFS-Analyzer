...
def run_analysis(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"3. `## Field vs Sharp — how this slate gets played` — the head-to-head read the user "
        ...
        f"tier; the sharp gets the same ceiling at 10% in the $9K tier'). Descriptive throughout — "
        f"players and tiers cited as data, never as PLAY/FADE commands.\n"
        f"4. `## Top plays` — tiered **Core / Good / Okay** (same vocabulary as the Player pool; "
        ...
        f"7. `## How to approach the slate` — 3–4 lines only. **This tool is focused on small-field "
...
def run_player_pool(slug: str, contest_label: str, sport: str) -> dict:
    ...
    prompt = (
        ...
        f"user must understand every write-up with zero decoding. NEVER print a vendor term "
        f"(coffin, dock, boost, sim-optimal, steam) without translating it in the same sentence — "
        f"e.g. 'ETR is playing him 8 points MORE than the field expects (coffin +8.2)'. END the "
