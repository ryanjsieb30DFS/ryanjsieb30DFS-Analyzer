...
- **Leverage has a price, and Week 1 charges the most (ETR, Brott 9/12/26 — Milly Maker + The Spy data, large fields).** Among matched twins (same position, salary within $500, projection within one point, one at 2x+ the ownership of the other), the higher-owned twin outscored the lower-owned one about 60% of the time in Week 1 and reached 25 DK points more often. The lower-owned twin bought roughly one-quarter (Spy) to one-third (Milly) of the ownership at a real ceiling cost; later in the season that ceiling gap narrows a lot while the ownership gap stays. Read: the field's preference carries information — respect the ownership premium more in Week 1 and lean on the low-owned twin more as the season goes on. This prices leverage; it never fades it. The user's own small-field ledger wins any conflict.
...
- **DST — punt with intent.** Cheap, low-owned, never against your own stack; ideally attached to a favorite alongside its RB. The only hard rules at DST are the negative ones. **Grade a DST by QB pressure, never by opponent points allowed (Thorn, 9/12/26):** only 2.5% of games last season were shutouts and only 9.7% held a team to six points or fewer, so the points-allowed bonus is "a fool's errand." DK DST points come from sacks, fumbles, interceptions and touchdowns, and those come from the pass-rush mismatch — Thorn's weekly OL vs. DL ranks (opponent OL rank vs. own DL rank) are the input. The same mismatch cuts the other way: a QB stack whose line faces a top pressure mismatch is a weaker stack and a weaker bring-back source that week.
