...
> **Carry a low-owned leverage dart in MME.** Across a 10+ -bullet NASCAR MME portfolio, expect roughly one lineup to carry a genuinely low-owned leverage driver with real upside (a mid-pack flex or PD sleeper, not chalk relief) — about 1 in 14, a portfolio RATE, never one per lineup. Where no such driver has a live case this week, carrying none is correct. Threshold: **sub-10% own, tightening to sub-5% at high-chaos tracks** (Herbst 4% was in the #1 Chicagoland lineup; Byron 10-13% / Briscoe 6-10% / Gilliland 6% were the broader paying leverage). Scope: MME only — a sub-10% punt with a Dustin "MME or fade" tag is not a single-entry leverage play. Track every slate via `src/shark_gap.py` leverage_pct delta; the account's standing gap (-16.7 at Chicagoland) is the single biggest fixable NASCAR leak.
...
8. **Anchor-Equivalence:** For every chalk-tier (≥20% projected own) dominator candidate, identify the equivalent-profile alternative (within ±10% own, similar salary tier). **At least one lineup MUST run the alternative.** No exceptions — this has cost 4 consecutive slates across 2 sports — run the equivalent alternative in ≥1 lineup as *coverage*; do not hedge the dom coin flip to parity. Keep the modal dominator as anchor; size the alternative for leverage (1-2 lineups). **Drafting-track carve-out (2026-08-30):** at Daytona/Talladega/Atlanta chalk doms are a correlated stack, not substitutes — the substitution pass does not apply there; see rules/shared/anchor_equivalence.md and check #12.
9. **Sleeper-spike screen:** Identify the sub-15% own / sub-$6K drivers projected for ≥15 SP (place differential) and decide on each one deliberately. Charlotte 600 had 10 such plays score 23+ FPTS and the portfolio touched none — that miss was a FAILURE TO LOOK, which is what this check fixes. It is not a quota: if this race's cheap deep-starters have no real path, carrying zero is the correct answer. Price and ownership identify a candidate; they never qualify one.
...
11. **Definer hand-off — a CONSIDERATION gate, not a roster quota (codified 2026-08-09, NW + Indy + Iowa; rewritten 8/29/26 on user directive).** Before locking, take each driver on the strategy's sub-10% definer list and make an explicit, written call: in, or out and why. What the five-slate evidence actually shows is a LOOKING failure — the screen named slate-deciders every time (SVG/Suarez/Gilliland at NW; Heim/Keselowski at Indy; Nemechek/Stenhouse at Iowa, the latter two in both Iowa winners) while entered usage ran 0%, 50%, 0/12, 0/12, 0/12. Those definers went unconsidered, not considered-and-declined. **The old wording — "it must carry EXACTLY ONE driver from the sub-10% definer list" — is retired.** It forced a driver into every bullet on an ownership number alone, which is the rate-restated-as-quota error (`mma-se-2026-07-19-sharp-envelope-is-a-rate-not-a-per-bullet-quota`), and it contradicts this repo's own philosophy.md: *"'Sub-10% own' is necessary but not sufficient… the field is correctly identifying the sub-10% as bad — that's why it's at sub-10%."* It also named the Grade tab's no-leverage flag as its checkpoint; that flag has been info-only in every sport since 8/28/26 and enforces nothing. A bullet carrying zero definers is fine when each was judged and passed on.
12. **Anchors held, sixth slot differentiates (codified 2026-08-30, Indy + Iowa + Daytona).** An SE entry keeps every consensus anchor — the top 3 projected-ownership names — unless the Fades section states a CEILING case for dropping that driver (a reason his top-end score can't happen; an ownership-vs-projection rank gap is not one). Differentiation lives in the sixth slot, filled from the definer list, never in the anchor slots. Mid-owned (15-40%) pivots in anchor slots are the fish shape: too owned to be unique, not the anchors, structurally dominated by chalk-plus-one-definer (Indy: sharks 100% on all 3 anchors, my entry kept 1; Iowa: anchors dropped with no case, 64.8 percentile; Daytona: both entries dropped 2 of 3 anchors for Blaney/Elliott/Bubba, both winners held all 3 plus a 10-13% sixth). See [[nascar-2026-07-26-anchors-held-differentiate-in-sixth-slot]]. **Scope note (2026-09-11, Darlington):** this binds the per-contest picker too — a pick rationale that drops a top-three anchor on a rank gap is a rejected override.
13. **Player-pool tiering — the Fade tier is for negative-leverage chalk and cars with no path to a top-15 (codified 2026-09-11, Indy + Daytona + Darlington).** A sub-10%-owned driver the articles tag "MME or fade" / "large-field-only" / "random contrarian" is tiered Okay · Leverage, never Fade — that phrase prices his floor, and the SE payout pays his ceiling. See [[nascar-2026-07-26-fade-tier-buries-ceiling-tagged-definers]].
If check 8, 10, or 12 fails, **swap the offending lineup before locking**. If check 13 fails, re-tier the driver before the board is handed off. Checks 9 and 11 are CONSIDERATION checks — they fail only when a candidate was never looked at, and they are satisfied by a deliberate "no," never by adding a driver to clear a threshold.
...
- **Sub-15% play → usually a pivot, rarely an anchor** (judge the car, not the ownership number)
...
- **HMS + intermediates → consider double-up** (Texas confirmed). An article org-narrative that demotes HMS at an intermediate lowers the field's pick rate, not the cars' ceiling — treat the low-owned HMS driver as leverage, not a fade (Chicagoland: Byron 85.25 at 10-13%; merged 2026-09-11).
