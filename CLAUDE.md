...
| `src/metrics_registry.py` | Read the per-sport custom-metric registry `rules/<slug>/metrics.yaml` (`list_metrics`/`get_metric`) — metrics are authored in SaberSim, recorded here (definition + `export_column` + changelog) |
| `src/metric_tracker.py` | **Grade a custom metric** (Sim Data tab): from a POST-race SaberSim export (custom-metric column + populated `Actual`), `grade_metric` computes full-pool Spearman (rank-corr, no scipy) + decile lift + best-lineup metric percentile; `log_performance`/`load_performance` over `rules/<slug>/metric_performance.jsonl`. Analytics only — never builds lineups |
