...
8. `## Optimizer build rules` — LAST section: 3–6 **concise, copy-paste-ready** rules in the Grader Lineup Builder's EXACT vocabulary, a **FAITHFUL TRANSLATION of the strategy you just wrote + the uploaded data — NOT generic heuristics.** Hard rules: **(a)** every rule traces to a specific `## Decisions` / `## Leverage & fades` / `## Top plays` line or a cited data point — no invented constraints; **(b)** SALARY comes from THIS slate's optimal-lineup evidence, NEVER a generic "max salary" (maxing is a golf/MMA trait, WRONG for NASCAR PD builds — never set a floor that excludes the slate's actual optimals; if the body says salary non-binding, use a low/no floor, and the number must agree with its reason); **(c)** each rule must not contradict the body or another rule (no "non-binding" + a high floor; don't over-constrain into infeasibility); **(d)** Player Groups use the exact players the Decisions name, and a deprioritized headline play (a chalk "GPP Fav") is noted, not silently dropped. Rule types: **Salary band** (`Min $X / Max $Y`); **Player Groups** (`At Least|At Most|Exactly N from: <names>`); **Restrictions** (`At Least|At Most|Exactly N with [Proj|Ceiling|Own %|Salary|Starting Pos] [Less Than|Greater Than|Equals|Between] <value>`); **IF-THEN**; **Min Uniques** / **Exposure %**. One rule/line + a ≤10-word why naming its source; never a lineup.
