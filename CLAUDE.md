...
Five tabs: **Projections → Slate Data → Slate Strategy → ✅ Grade → Autopsy**. The **Grade** tab is a pre-lock checker for HAND-BUILT lineups: the user pastes the lineups they built in DK and gets an instant, sport-calibrated grade (every threshold data-derived — shark envelope, own-contest winners, strategy-contract fades, board tiers, recurring dupe-magnet pairs — never a hardcoded universal number), plus an optional `claude -p` thesis check (one-line "how it wins" per lineup, portfolio distinctness, lesson activation). **It grades and names weaknesses only — it NEVER builds, selects, swaps, or fixes lineups.** (The former **Sim Data**, **Post-Contest Sim Data**, and **Trends** tabs were removed — the first two with the SaberSim retirement, Trends as part of the strip-down; their modules are dormant on disk, see below.) The Slate Strategy tab also has a **Player pool** below the written strategy — a Claude-generated, ranked, annotated board of every rosterable player, each tiered **Core / Good / Okay / Fade** (+ an orthogonal `· Leverage` label) with a short GPP write-up. This board is the primary build reference. Built automatically as part of the slate-strategy generation (no separate button).
...
| `src/grader.py` | **✅ Lineup grader** (Grade tab) — sport-calibrated pre-lock checks for pasted hand-built lineups. Calibration is ALL data-derived: ownership target from the sport's `shark_baseline.json` envelope + your logged contests' `winners_avg_own` (flag only above target × 1.2); the no-leverage flag fires ONLY where the sport's sharks actually carry sub-5% pieces (`leverage_pct` ≥ 30 — MMA/NASCAR pros run chalk, so there it stays info-only); fade violations from the strategy contract; bottom-tier hits from the board; recurring crowded-pair (dupe-magnet) warnings; expected-dupes from the ownership product × declared field; portfolio checks (all-unique + competing-lineup overlap). Draft-persisted input; `analysis_runner.run_grade` adds the claude thesis check → `data/grade/<slug>.md` (archived as `lineup_grade.md`, cleared with the slate). **Self-validating:** `retro_grade` auto-grades the ENTERED lineups at every autopsy log (same calibrated gates vs actual ownership) → `<history_dir>/grader_validation.json` + `grader_check` in results.jsonl; the process trend reports whether flagged lineups actually finish worse than clean ones (and says "recalibrate" when they don't). Grades only — never builds/swaps/fixes |
...
| `src/field_analysis.py` | **Field / Fish autopsy analysis** (standings-only, the inverse of shark_gap): `field_profile` surfaces where the WHOLE FIELD converges (crowded chalk players, crowded pairs, exact-roster dupe magnets) AND the FISH specifically (bottom-finishers vs top-1% winners: own/slot, dart rate, unique %, + `fish_traps` = players loved by losers, faded by winners). Rendered as a "🐟 leverage away from this" fade board in the Autopsy tab. Analytics only — never builds lineups |
| `src/field_tendencies.py` | **Cumulative field-tendencies store** (`rules/<slug>/field_tendencies.jsonl`, append-only, NOT cleared): each logged autopsy appends one row per contest carrying `contest_type`, `contest_name`/`contest_key` (the specific recurring contest), and a `contest_id` (the DK contest instance). `_load` **dedups by `contest_id`** — re-logging the same standings collapses to the latest row, so the "in N of M" reliability counts never inflate. Two rollups: `summarize` (by type) and the sharper `summarize_contest` (by name — same contest = same field, adds recurring opponents + sharpness trend), both showing "the field reliably crowds X / Y is a recurring trap" in the autopsy panel when ≥2 prior contests exist. `bundle_block` **forward-feeds** that synthesis into the next slate's bundle (the `## Field tendencies` section) so the slate strategy leverages AWAY from the crowd; `crowded_names` backs the app's field-tendency coverage-gap warning. Never a fade command — synthesis only |
| `src/shark_accumulate.py` | **Living shark envelope**: each autopsy appends the in-field sharks' observed `structural_profile` (own/slot, leverage %, anchor exposure, unique %) to `rules/shared/shark_observations.jsonl`, then `refresh_baseline()` recomputes `rules/shared/shark_baseline.json`'s per-sport `shark_envelope` as a count-weighted blend of the frozen 63-contest seed + accumulated observations (idempotent — seed frozen). The Sim tool reads the refreshed envelope so accumulated shark structure flows into its Diversifier's shark-match target. Best-effort; never blocks the autopsy log |
...
**Entry-cap focus — SE / 3-Max / 5-Max ONLY.** This tool is scoped to small-field GPPs: Single Entry, 3-Max, and 5-Max (`src/contests.py::CONTEST_TYPES` / `FOCUS_CONTEST_TYPES`). Everything downstream optimizes for a tight all-unique set of 1/3/5 bullets — strategy framing, the sharp-envelope target, and the shark benchmark (which accumulates ONLY SE/3/5-max shark observations, never their 150-max MME dumps). The 150-max MME game is intentionally out of scope; don't frame reads or rules for it.
...
**Leverage-coverage rule:** when projections are loaded, the bundle includes a `## Leverage candidates to address` section — the sub-10%-owned, high-ceiling plays (`src/landscape.py::leverage_candidates`). **Every** listed player MUST be ADDRESSED in the slate strategy's `## Leverage & fades` or `## Edges & tensions` with a one-line synthesis of their leverage/ceiling case (surface it — no play/fade command required), and the Player pool must confirm each in its `## Leverage candidates addressed` footer (ranked, `· Leverage`-labeled — or tiered `Fade` with a reason). A sub-10% high-ceiling play left unmentioned is a coverage leak — the play that decides the slate from nowhere (the Kaan Ofli failure, 6/27 Baku). The app shows a **coverage-gap warning** if the rendered strategy omits any candidate.

**Field-tendency coverage rule:** when the bundle includes a `## Field tendencies` section (≥2 logged autopsies of the contest/type — `src/field_tendencies.py::bundle_block`), the slate strategy MUST surface every reliably-crowded cluster in `## Edges & tensions` as a leverage-away tension citing the in-N-of-M counts, and the Player pool MUST tag every listed crowd player on the board with `(field crowds)`. Both stay descriptive — surface where the field piles in so the user can leverage away; issue no fade command. The app shows a **field-tendency coverage-gap warning** if the rendered strategy names none of the crowded plays.

### Slate strategy format (mandatory, all sports)

**SYNTHESIS-FIRST, tight, scannable.** **This tool ORGANIZES and SYNTHESIZES the data — it does NOT tell the user who to play.** Surface edges, tensions, mispricings, and rankings; the user makes every play/fade/build decision and builds in SaberSim. No imperative play/fade commands, no roster shaping. **NO pre-flight/checklist section is printed** — that prep (venue file, open lessons, framework pre-lock checks) happens silently and only its RESULT shows below. **Hard writing rules, all sections:** every bullet ≤~25 words, one idea/line, bold the key noun, no prose blocks; **cite everything** (every claim names a specific article line OR a projection number); short tables where they compress. Sections, in this order:

1. `## Slate at a glance` — ≤6 lines: games/fights/races, implied totals/win probs, weather, contests + field sizes. A short table. (If the slate looks stale, instead open the doc with a single bold `⚠️` warning line and stop.)
2. `## Edges & tensions` — **THE STAR, up top.** The 3–6 things that actually matter this slate, SYNTHESIZED — **NOT play/fade calls.** ONE line each: **the edge/tension** · the cited data that reveals it. Surface: where scoring/leverage concentrates, ownership-vs-ceiling mispricings, where vendors/articles DISAGREE, substitutable chalk clusters. **Anchor-Equivalence MUST appear** — as a surfaced observation ("X and Y near-identical at similar own"), never "run the alternative." **If the bundle has a `## Field tendencies` section, EVERY reliably-crowded cluster it lists MUST be surfaced here too** — one line each, "the field piles into X/Y in your <type> contests (in 3 of 4) — leverage lives away from these," citing the in-N-of-M counts (surface it; never "fade X"). NO imperative verbs (PLAY/FADE/run/cap/pair/build) — state the edge and stop; the user decides.
3. `## Top plays` — tiered **Core / Good / Okay** (same vocabulary as the Player pool; Fades live in §4), one line each: **player** ($sal, own% per source) — tier — ≤12-word cited why; append `· Leverage` to any low-owned high-ceiling play.
4. `## Leverage & fades` — two parts: **(a) Low-owned definers (MANDATORY screen)** — for EVERY game/fight/race name the single sub-10%-owned high-ceiling play (INCLUDING ones the articles never named) + its one-line ceiling path — the non-negotiable fix for the recurring "faded the winning sub-10% definer" miss; every spot gets a line. **(b)** a `**Fades:**` sub-header, then chalk worth fading, each player + verdict (FADE/LEAN FADE/UNDERWEIGHT) + the world it needs. (extract_fades / strategy_contract depend on the `## Leverage & fades` name + the `**Fades:**` sub-header — keep both.)
5. `## Key themes` — ≤3 bullets: structural storylines + where sources DISAGREE (that gap is the edge).
6. `## How to approach the slate` — 3–4 lines: winning shape, chalk-vs-contrarian lean (SE/3/5-max small-field focus), sharp-envelope target (≥1 sub-5% piece, ceiling over median, all-unique). No restating plays.
