...
Seven tabs: **Projections → Slate Data → Slate Strategy → Sim Data → Post-Contest Sim Data → Autopsy → Trends**. **Sim Data** ingests a SaberSim lineup-pool export and surfaces, for any sport, the sim's good plays / bad plays, sim-vs-field leverage, and **chalky combinations** of players — **analytics only, it NEVER builds or picks lineups** (combinations are duplication/leverage signal, not lineups to play). **Post-Contest Sim Data** holds the custom-metric workflow: the metric **definition** card (read from `rules/<slug>/metrics.yaml`) and the post-race **grading** of that metric (Spearman/decile-lift vs DK actuals + the performance-trend ledger) — analytics only, never lineups. Trends is a cross-sport, read-only "where you win" view — best-percentile by contest type + field size. The Slate Strategy tab also has a **Player pool** below the written strategy — a Claude-generated, ranked, annotated board of every rosterable player, each tiered **Core / Good / Okay / Fade** (+ an orthogonal `· Leverage` label) with a short GPP write-up. This board is the primary build reference. Built automatically as part of the slate-strategy generation (no separate button).
...
| Path | Purpose |
|---|---|
| `app.py` | Streamlit UI: 7 tabs (Projections, Slate Data, Slate Strategy, Sim Data, Post-Contest Sim Data, Autopsy, Trends) |
...
| `src/field_tendencies.py` | **Cumulative field-tendencies store** (`rules/<slug>/field_tendencies.jsonl`, append-only, NOT cleared): each logged autopsy appends one row per contest keyed by `contest_type`; `summarize` rolls the history into "across N past <type> contests, the field reliably crowds X / Y is a recurring trap," shown in the panel when ≥2 prior contests of that type exist. The "moving forward" substrate for building away from the crowd (auto-feeding it into slate strategy/builds is a deferred follow-up) |
...
| `src/sim_data.py` | **Sim Data tab** analytics (deterministic, all sports): parse a SaberSim lineup-pool CSV (`load_sim_pool` — auto-detects roster slots), per-player `player_exposure` (sim exposure % + projections join: field/projected ownership, proj_points, ceiling, salary), `good_bad_plays` (**projection-ownership leverage**: good = underowned value [high upside, low field own], bad = overowned chalk [owned past what the upside earns]; upside = ceiling when real else proj_points; sim exposure is context only, NOT the driver — plus the separate sim-vs-field `leverage`/`trap`), `chalky_combinations` (top co-occurring pairs/trios = duplication risk). **Analytics only — never assembles a lineup.** |
...
| `src/strategy_contract.py` | **Analyzer → Sim-tool hand-off**: on strategy generation, writes `data/strategy_contract/<slug>.json` — per-player calls (play/pass/pass_mix/underweight/lean_fade/fade) parsed by VERDICT from the `## Leverage & fades` section (NOT `extract_fades`, which sweeps the whole section when there's no literal "**Fades**" subheading) + leverage candidates. The Sim tool (`~/Desktop/Repo/ryanjsieb30DFS`) displays the calls and auto-applies hard fades only. Cleared + archived with the slate. The reverse hand-off: the Sim tool writes its pool export directly to `data/sim_data/<slug>__pool.csv` (Sim Data tab auto-detects) |
| `src/metrics_registry.py` | Read the per-sport custom-metric registry `rules/<slug>/metrics.yaml` (`list_metrics`) — metrics are authored in SaberSim, recorded here (definition + `export_column` + changelog); rendered as the **definition card** in the Post-Contest Sim Data tab |
| `src/metric_tracker.py` | **Grade a custom metric** (Post-Contest Sim Data tab): from a POST-race SaberSim export (custom-metric column + populated `Actual`), `grade_metric` computes full-pool Spearman (rank-corr, no scipy) + decile lift + best-lineup metric percentile; `log_performance`/`load_performance` over `rules/<slug>/metric_performance.jsonl`. Analytics only — never builds lineups |
