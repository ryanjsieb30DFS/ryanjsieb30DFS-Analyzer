...
| `src/metrics_registry.py` | Read the per-sport custom-metric registry `rules/<slug>/metrics.yaml` (`list_metrics`/`get_metric`) — metrics are authored in SaberSim, recorded here (definition + `export_column` + changelog); rendered as the **definition card** in the Post-Contest Sim Data tab |
