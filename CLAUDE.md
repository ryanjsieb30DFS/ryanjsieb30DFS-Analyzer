...
| `src/player_pool.py` | **Player pool / fighters-ranked board** (Slate Strategy tab): a Claude-generated **ranked board — an easy-to-read TABLE first** (Rank/Player/Tier/Sal/Proj[/Ceiling/Win% for MMA]/Own/How-it-wins via `app._split_leading_table`), write-ups in an expander below. Membership = the rosterable universe from the loaded projections (`build_pool`, which carries `ceiling` — MMA uses `proj_win` = points-if-they-win — plus `win_prob` + `opponent`-from-`matchup`) **minus** the strategy's fades (`extract_fades`/`apply_fades`) WHEN a strategy exists; without one it ranks the **full pool**. `analysis_runner.run_player_pool` runs `claude -p` to rank + write up from the documents (projections + ownership + articles + framework). Runs automatically after `run_analysis`, OR **standalone** via the "🏆 Rank players (all data)" button (no strategy required — a one-click board any time projections are loaded). Persisted at `data/player_pool/<slug>.md`; individual-player ranking only — NEVER builds lineups |
...
**DECISION-FIRST, tight, scannable** — a hand-builder's cheat sheet, NOT an essay (built from in ~60s). **Hard writing rules, all sections:** every bullet ≤~25 words, one idea/line, bold the player/decision, no prose blocks; **cite everything** (every play/leverage/fade names a specific article line OR a projection number — no framework-only justification); short tables where they compress. Sections, in this order:

1. `## Pre-flight checklist` — first, always (see Pre-flight ritual). Terse.
2. `## Slate at a glance` — ≤6 lines: games/fights/races, implied totals/win probs, weather, contests + field sizes. A short table.
3. `## Decisions` — **THE STAR, up top.** 3–6 PLAY/PASS/MIX calls in priority order, ONE line each: **the call** · ≤15-word cited why · build shaping (`→ pair with / cap at / thin / core`). **Anchor-Equivalence call MUST be one.** This is what the user builds from.
4. `## Top plays` — tiered (core / pivots / darts), one line each: **player** ($sal, own% per source) — ≤12-word cited why.
5. `## Leverage & fades` — two parts: **(a) Low-owned definers (MANDATORY screen)** — for EVERY game/fight/race name the single sub-10%-owned high-ceiling play (INCLUDING ones the articles never named) + its one-line ceiling path — the non-negotiable fix for the recurring "faded the winning sub-10% definer" miss; every spot gets a line. **(b)** a `**Fades:**` sub-header, then chalk worth fading, each player + verdict (FADE/LEAN FADE/UNDERWEIGHT) + the world it needs. (extract_fades / strategy_contract depend on the `## Leverage & fades` name + the `**Fades:**` sub-header — keep both.)
6. `## Key themes` — ≤3 bullets: structural storylines + where sources DISAGREE (that gap is the edge).
7. `## How to approach the slate` — 3–4 lines: winning shape, chalk-vs-contrarian lean (field-size aware), sharp-envelope target (≥1 sub-5% piece, ceiling over median, all-unique). No restating plays.
8. `## SaberSim build rules` — LAST. 4–8 **copy-paste-ready** rules the user types into SaberSim to build every lineup — a **FAITHFUL TRANSLATION of the Decisions / Leverage & fades / Top plays + vendor data, NOT generic heuristics.** Primitives ONLY (one/line): `LOCK: <player>` (core, every lineup) · `EXCLUDE: <player>` (**hard FADE only**) · `GROUP — At Least|At Most|Exactly N of: <players>` (Rules-modal quantifier) · `POOL FILTER — Show players with|without <My Own|My Proj|Value|Salary|Win %|…> <comparator> <value>` (vendor-native stats only) · `SALARY: Min $X / Max $Y` **only if this slate's optimal-lineup evidence supports it**, else literally `SALARY: leave to SaberSim (Sim mode optimizes it)`. **Discipline:** (a) every rule traces to a Decisions/Leverage/Top-plays line or a cited vendor number, ending `— ≤10-word why`; (b) vendor projections/ownership/data ONLY; (c) **Anchor-Equivalence → `GROUP — At Most 1 of: <anchors>`**; (d) hard FADE→`EXCLUDE`, LEAN FADE/UNDERWEIGHT→`GROUP — At Most N of`/pool filter (never Exclude); (e) sharp leverage → `GROUP — At Least 1 of: <sub-5% pieces>`; (f) SALARY from this slate's data or deferred — **never a generic max-salary floor** (the NASCAR-PD failure); (g) no rule contradicts the body/another rule; never a lineup, only individual players + constraints. This section is additive + last — it does NOT disturb the decision-first order or the `## Leverage & fades`/`**Fades:**` subsection.
