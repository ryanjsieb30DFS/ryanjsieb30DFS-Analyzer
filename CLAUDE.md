...
4. **Analyze** — declare contests, review the auto-snapshot (chalk tiers, leverage, anchor-equivalence, sport signals, vendor disagreement), then click **Generate slate analysis** — the app builds the bundle and runs `claude -p` headlessly to write + render the analysis (no chat needed). Two sibling lineup actions then sit below the analysis: **🏗️ Build lineups** (Claude invents the portfolio from scratch) and **🎯 Select lineups** (Claude picks the best lineups FROM the SaberSim lineup-export CSV uploaded in the Sim Data tab — see "Selecting lineups"). Select lineups is gated on a slate analysis existing AND a sim-data file existing AND the loaded pool carrying `dk_id`
...
The Analyze tab's **🎯 Select lineups** button (sibling to Build lineups) calls `src/analysis_runner.py::run_select_lineups`, which runs `claude -p` headlessly to **pick** the best lineups FROM the SaberSim lineup-export CSV the user uploaded in the Sim Data tab — it does NOT invent rosters. It requires BOTH a slate analysis (`data/slate_analysis/<slug>.md`) and a sim-data file to exist, and the loaded projections pool to carry `dk_id` (the SaberSim **projections** vendor supplies it; SIN MLB has no IDs). Each SaberSim CSV row is one candidate lineup: the first N columns are DK player IDs in `roster_spec(slug)` slot order (MLB: P,P,C,1B,2B,3B,SS,OF,OF,OF), followed by metric columns (Proj Score, percentile ceilings, Ownership, Salary, Saber Score, per-contest ROI/Win Rate/Cash Rate/Sim Dupes). The run resolves each ID to a player via the pool's `dk_id` column, then **FILTERS to lineups that express the slate analysis's edges and Player-board calls — NOT SaberSim's top-ROI/Saber-Score rows** (sim rank is explicitly NOT a quality filter). It SELECTS up to `unique_lineups_needed` lineups that answer DISTINCT questions, enforcing the same discipline as Build lineups (Anchor-Equivalence, never a hitter against your own pitcher, GPP framing throughout) and writes `data/lineups/<slug>.md` in the EXACT portfolio format Build lineups uses (pre-flight checklist first; each lineup tagged as selected from the uploaded SaberSim pool with its source CSV row index for traceability; Portfolio audit at the end). It writes ONLY `data/lineups/<slug>.md` — never the analysis, bundle, or sim file. Selected lineups are reviewed by 🔪 Red team and archived on autopsy like any other portfolio lineup.
