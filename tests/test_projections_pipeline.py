"""Data-pipeline tests (Tier-3): ownership scaling, stddev derivation, vendor
detection confidence, and landscape functions on empty/tiny pools."""
import sys
...
def test_landscape_handles_empty_and_tiny_pools():
    empty = pd.DataFrame({"name": [], "ownership": [], "proj_points": [],
                          "salary": [], "ceiling": []})
    # Must not raise on an empty pool.
    assert landscape.anchor_equivalence_check(empty) == []
    assert landscape.leverage_table(empty).empty
    one = pd.DataFrame({"name": ["A"], "ownership": [30.0], "proj_points": [10.0],
                        "salary": [8000], "ceiling": [20.0]})
    assert landscape.anchor_equivalence_check(one) == []  # need >=2 chalk to group
